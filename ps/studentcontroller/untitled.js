...
var TempRA = require('../../../models/temp_ra');
...
exports.post = function(req, res) {
    ...
    var tempPromise = TempRA.find().exec()
    ...
    var stunotfind = function(temp) {
            console.log("temp", temp)
            return new Promise(function(resolve, reject) {
                notfind = temp.map(function(data) {

                    RA.findOne({
                        $or: [{ "email.primary": data.email[0].primary },
                            { "email.alternate": data.email[0].primary }, {
                                $and: [{ firstName: data.firstName },
                                    { lastName: data.lastName },
                                    { "mother.motherFirstName": data.mother[0].motherFirstName },
                                    { "mother.motherLastName": data.mother[0].motherLastName },
                                    { "dateOfBirth.dob": data.dateOfBirth[0].dob }
                                ]
                            }
                        ]
                    }, function(err, pri) {
        ...
        //console.log("not find",notfind)


        var stufind = function(temp) {
            console.log("temp", temp)
            return new Promise(function(resolve, reject) {
                notfind = temp.map(function(data) {
...
//                     RA.findOne({ "email.primary": data.email[0].primary }, function(err, pri) {
//                             console.log("pri", pri)

//                             if (pri == null || pri == undefined || pri == '') {
//                                 RA.findOne({ "email.alternate": data.email[0].primary }, function(err, mail) {
//                                     console.log("alter", mail)
//                                     if (mail == null || mail == undefined || mail == '') {
//                                         RA.findOne({ $and: [{ firstName: data.firstName }, { lastName: data.lastName }, { "mother.motherFirstName": data.mother[0].motherFirstName }, { "mother.motherLastName": data.mother[0].motherLastName }, { "dateOfBirth.dob": data.dateOfBirth[0].dob }] }, function(err, name) {
